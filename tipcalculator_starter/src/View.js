...
const formatMoney = R.curry((symbol, places, number) => {
  return R.pipe(R.defaultTo(0), round(places), num => num.toFixed(places), R.concat(symbol))(number)
})
...
function calculatedAmounts(tip, total) {
  return div({ className: 'w-40 b bt mt2 pt2' }, [calculatedAmount('Tip:', tip), calculatedAmount('Total:', total)])
}
...
function view(dispatch, model) {
  ...
  return div({ className: 'mw6 center' }, [
    ...
    inputSet('Tip %', tipPercent, e => dispatch(tipPercentInputMsg(e.target.value))),
